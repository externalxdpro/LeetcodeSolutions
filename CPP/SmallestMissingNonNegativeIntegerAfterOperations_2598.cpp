...
class Solution {
  public:
    int findSmallestInteger(std::vector<int> &nums, int val) {
        std::vector<int> mods(val);
        for (int n : nums) {
            mods[(n % val + val) % val]++;
        }
        int result = 0;
        for (; mods[result % val] > 0; result++) {
            mods[result % val]--;
        }
        return result;
    }
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.findSmallestInteger(test.first, test.second);
