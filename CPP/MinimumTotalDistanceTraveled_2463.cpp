...
//     they do not collide. They cross each other. If a robot passes by a
//     factory that reached its limits, it crosses it as if it does not exist.
...
// does not need to move. The limit of the first factory is 2, and it fixed 2
// robots. The limit of the second factory is 2, and it fixed 1 robot. The total
// distance is |2 - 0| + |2 - 4| + |6 - 6| = 4. It can be shown that we cannot
// achieve a better total distance than 4.
...
// repaired at the first factory. The limit of the first factory is 1, and it
// fixed 1 robot. The limit of the second factory is 1, and it fixed 1 robot.
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<
        std::pair<std::vector<int>, std::vector<std::vector<int>>>, long long>>
        tests = {
            {{{0, 4, 6}, {{2, 2}, {6, 2}}}, 4},
            {{{1, -1}, {{-2, 1}, {2, 1}}}, 2},
        };
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.minimumTotalDistance(test.first, test.second);
