...
//     sorted in non-decreasing order. timei is the time needed to prepare the
//     order of the ith customer.
  ...
  public:
    double averageWaitingTime(std::vector<std::vector<int>> &customers) {
        ...
        for (std::vector<int> &customer : customers) {
            ...
            times += currTime + time - arrival;
            currTime += time;
        }

        return times / customers.size();
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.averageWaitingTime(test);
