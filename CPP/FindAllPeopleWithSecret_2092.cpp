...
class Solution {
  public:
    std::vector<int> findAllPeople(int                            n,
        ...
        while (!q.empty()) {
            const auto [time, person] = q.top();
            q.pop();

            if (dists[person] != INT_MAX) {
                continue;
            }
            result.insert(person);
            dists[person] = time;

            for (const auto &[newTime, newPer] : adj[person]) {
                if (dists[newPer] != INT_MAX || newTime < time) {
                    continue;
                }

                q.emplace(newTime, newPer);
            }
        }
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<std::tuple<int, std::vector<std::vector<int>>, int>,
                          std::vector<int>>>
        tests = {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.findAllPeople(
