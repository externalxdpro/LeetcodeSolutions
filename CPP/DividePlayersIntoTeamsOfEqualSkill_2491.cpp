...
// each team has a total skill of 6. The sum of the chemistry of all the teams
...
class Solution {
  public:
    long long dividePlayers(std::vector<int> &skill) {
        ...
        long long result = skill.front() * skill.back();
        for (int i = 1; i < skill.size() / 2; i++) {
            if (skill[i] + skill[skill.size() - i - 1] !=
                skill.front() + skill.back()) {
                return -1;
            }
            result += skill[i] * skill[skill.size() - i - 1];
        }
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.dividePlayers(test);
