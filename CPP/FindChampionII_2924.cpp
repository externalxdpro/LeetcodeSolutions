...
//     range [1, n]. A DAG is a directed graph that does not have any cycle.
...
//     b is not stronger than team a. The input is generated such that if team a
//     is stronger than team b and team b is stronger than team c, then team a
...
class Solution {
  public:
    int findChampion(int n, const std::vector<std::vector<int>> &edges) {
        ...
        int *pos = std::find(std::begin(counts), std::begin(counts) + n, 0);
        if (std::find(pos + 1, std::begin(counts) + n, 0) != counts + n) {
            return -1;
        }
        return pos - counts;
    }
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.findChampion(test.first, test.second);
