...
class Solution {
  public:
    int minimumSize(const std::vector<int> &nums, int maxOps) {
        int l = 1, r = 0;
  ...
  private:
    bool isPossible(int maxBalls, const std::vector<int> &nums, int maxOps) {
        int total = 0;

        for (int num : nums) {
            int ops = std::ceil((double)num / (double)maxBalls) - 1;
            total += ops;
            if (total > maxOps) {
                return false;
            }
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.minimumSize(test.first, test.second);
