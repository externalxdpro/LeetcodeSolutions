...
//     word. The last character of the last word is equal to the first character
//     of the first word.
...
//     spaces. The words in sentence are separated by a single space. There are
...
class Solution {
  public:
    bool isCircularSentence(const std::string &s) {
        auto it = std::find(s.begin(), s.end(), ' ');
        for (; it != s.end(); it = std::find(it + 1, s.end(), ' ')) {
            if (*(it - 1) != *(it + 1)) {
                return false;
            }
        }
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<std::string, bool>> tests = {
        ...
        {"Leetcode eisc cool", false},
    };

    Solution solution;
    for (auto &[test, ans] : tests) {
        auto result = solution.isCircularSentence(test);
