...
//     time t = 3. Preparation of the 1st potion begins at time t = 1, and is
//     completed by time t = 4. Preparation of the 2nd potion begins at time t =
//     2, and is completed by time t = 5.
  ...
  public:
    long long minTime(std::vector<int> &skill, std::vector<int> &mana) {
        ...
        for (int j = 0; j < m; j++) {
            for (int i = 0; i < n; i++) {
                if (i > 0) {
                    memo[i] = std::max(memo[i - 1], memo[i]);
                }
                memo[i] += skill[i] * mana[j];
            }
            for (int i = n - 1; i > 0; i--) {
                memo[i - 1] = memo[i] - skill[i] * mana[j];
            }
        }
        return memo.back();
    }
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.minTime(test.first, test.second);
