...
class Solution {
  public:
    Node *copyRandomList(Node *head) {
        ...
        Node *curr = head, *clone = new Node(head->val);
        clones[head] = clone;

        while (curr != nullptr) {
            if (curr->next != nullptr) {
                if (!clones.contains(curr->next)) {
                    clones[curr->next] = new Node(curr->next->val);
                }
                clone->next = clones[curr->next];
            }
            if (curr->random != nullptr) {
                if (!clones.contains(curr->random)) {
                    clones[curr->random] = new Node(curr->random->val);
                }
