...
class Solution {
  public:
    TreeNode *replaceValueInTree(TreeNode *root) {
        std::queue<std::pair<TreeNode *, TreeNode *>> q{{{root, nullptr}}};

        while (!q.empty()) {
            std::vector<std::pair<TreeNode *, TreeNode *>> nodes;
            int                                            sum = 0;
            for (size_t n = q.size(); n > 0; n--) {
                ...
                if (l != nullptr &&
                    ...
                    q.emplace(l->left, l->right);
                }
                if (r != nullptr &&
                    (r->left != nullptr || r->right != nullptr)) {
                    q.emplace(r->left, r->right);
                }
                nodes.emplace_back(l, r);
                sum += getVal(l) + getVal(r);
            }
            for (auto &[l, r] : nodes) {
                int curr = sum - getVal(l) - getVal(r);
                if (l != nullptr) {
                    l->val = curr;
                }
                if (r != nullptr) {
  ...
  private:
    int getVal(TreeNode *node) { return !node ? 0 : node->val; }
};
