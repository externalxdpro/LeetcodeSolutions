...
// array nums becomes: [1,2,1] -> [2,2,2]. The total sum of values is 2 + 2 + 2
// = 6. It can be shown that 6 is the maximum achievable sum of values.
...
// XOR 7 = 4, and the array nums becomes: [2,3] -> [5,4]. The total sum of
// values is 5 + 4 = 9. It can be shown that 9 is the maximum achievable sum of
// values.
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<
        std::tuple<std::vector<int>, int, std::vector<std::vector<int>>>, int>>
        tests = {
            {{{1, 2, 1}, 3, {{0, 1}, {0, 2}}}, 6},
            {{{2, 3}, 7, {{0, 1}}}, 9},
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.maximumValueSum(
