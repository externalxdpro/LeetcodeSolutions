...
// that there is exactly one person at every point. Alice wants to be alone with
// Bob, so Alice will build a rectangular fence with Alice's position as the
// upper left corner and Bob's position as the lower right corner of the fence
// (Note that the fence might not enclose any area, i.e. it can be a line). If
// any person other than Alice and Bob is either inside the fence or on the
...
// With Alice at (1, 3) and Bob at (1, 1), Bob's position is not the lower right
// corner of the fence.
...
// the lower right corner. Hence we return 0. Example 2:
...
// 4) will be inside the fence. Example 3:
...
// 1) will be on the fence. Note that it does not matter if the fence encloses
// any area, the first and second fences in the image are valid.
