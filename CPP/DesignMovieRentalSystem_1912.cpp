...
//     movie. The shops should be sorted by price in ascending order, and in
//     case of a tie, the one with the smaller shopi should appear first. If
//     there are less than 5 matching shops, then all of them should be
//     returned. If no shop has an unrented copy, then an empty list should be
//     returned. Rent: Rents an unrented copy of a given movie from a given
//     shop. Drop: Drops off a previously rented copy of a given movie at a
//     given shop. Report: Returns the cheapest 5 rented movies (possibly of the
//     same movie ID) as a 2D list res where res[j] = [shopj, moviej] describes
//     that the jth cheapest rented movie moviej was rented from the shop shopj.
//     The movies in res should be sorted by price in ascending order, and in
//     case of a tie, the one with the smaller shopj should appear first, and if
//     there is still tie, the one with the smaller moviej should appear first.
//     If there are fewer than 5 rented movies, then all of them should be
//     returned. If no movies are currently being rented, then an empty list
//     should be returned.
...
//     unrented copy of the given movie as described above. void rent(int shop,
//     int movie) Rents the given movie from the given shop. void drop(int shop,
//     int movie) Drops off a previously rented movie at the given shop.
...
// [1], [0, 1], [1, 2], [], [1, 2], [2]] Output [null, [1, 0, 2], null, null,
// [[0, 1], [1, 2]], null, [0, 1]]
...
class MovieRentingSystem {
    std::map<std::pair<int, int>, int> shops;
    std::unordered_map<int, std::set<std::pair<int, int>>> movies;
    std::set<std::tuple<int, int, int>> rented;

  public:
    MovieRentingSystem(int n, const std::vector<std::vector<int>> &entries) {
        for (auto &e : entries) {
...
int main(int argc, char *argv[]) {
    MovieRentingSystem *obj = new MovieRentingSystem(
        3, {{0, 1, 5}, {0, 2, 6}, {0, 3, 7}, {1, 1, 4}, {1, 2, 7}, {2, 1, 5}});
    fmt::println("{}", obj->search(1));
    obj->rent(0, 1);
    obj->rent(1, 2);
    fmt::println("{}", obj->report());
    obj->drop(1, 2);
    fmt::println("{}", obj->search(2));
