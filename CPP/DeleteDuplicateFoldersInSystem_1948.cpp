...
// because they both contain an empty folder named "y". Note that folders "/a"
// and "/c" are identical after the deletion, but they are not deleted because
...
//     For any folder not at the root level, its parent folder will also be in
//     the input.
...
class Solution {
  ...
  private:
    ...
    void insert(Node *root, std::vector<std::string> &path) {
        Node *node = root;
        for (auto &name : path) {
    ...
    void markDups(std::unordered_map<std::string, std::vector<Node *>> &map) {
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<std::vector<std::vector<std::string>>,
                          std::vector<std::vector<std::string>>>>
        tests = {
            {{{"a"}, {"c"}, {"d"}, {"a", "b"}, {"c", "b"}, {"d", "a"}},
             {{"d"}, {"d", "a"}}},
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.deleteDuplicateFolder(test);
