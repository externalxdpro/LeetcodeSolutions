...
//     amount of it. After distribution, each store will have been given some
//     number of products (possibly 0). Let x represent the maximum number of
//     products given to any store. You want x to be as small as possible, i.e.,
//     you want to minimize the maximum number of products that are given to any
//     store.
...
// amounts: 3, 3 The maximum number of products given to any store is max(2, 3,
// 3, 3, 3, 3) = 3.
...
// amounts: 5, 5 The maximum number of products given to any store is max(5, 5,
// 5, 5, 5, 5, 5) = 5.
...
class Solution {
  public:
    int minimizedMaximum(int n, std::vector<int> &quantities) {
        std::function<bool(int)> canDistribute = [&](int x) -> bool {
            int j   = 0;
        ...
        int l = 0, r = *std::max_element(quantities.begin(), quantities.end());
        while (l < r) {
            int m = (l + r) / 2;
            if (canDistribute(m)) {
                r = m;
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.minimizedMaximum(test.first, test.second);
