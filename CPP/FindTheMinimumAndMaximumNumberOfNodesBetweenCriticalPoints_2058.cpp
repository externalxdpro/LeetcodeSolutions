...
// and 2. The minimum distance is between the fifth and the sixth node.
// minDistance = 6 - 5 = 1. The maximum distance is between the third and the
...
// than 2 and 2. Both the minimum and maximum distances are between the second
// and the fifth node. Thus, minDistance and maxDistance is 5 - 2 = 3. Note that
// the last node is not considered a local maxima because it does not have a
// next node.
...
class Solution {
  public:
    std::vector<int> nodesBetweenCriticalPoints(ListNode *head) {
        ...
        ListNode        *bfr = head, *cur = head->next, *nxt = head->next->next;
        int              i = 1;
        std::vector<int> crit;

        while (nxt != nullptr) {
            if ((bfr->val < cur->val) == (cur->val > nxt->val) &&
                (bfr->val != cur->val) && (cur->val != nxt->val)) {
                crit.push_back(i);
            }
        ...
        for (int i = 0; i < crit.size() - 1; i++) {
            min = std::min(min, crit[i + 1] - crit[i]);
        }

        return {min, crit.back() - crit.front()};
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.nodesBetweenCriticalPoints(toLinkedList(test));
