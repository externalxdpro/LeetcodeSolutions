...
class Solution {
  ...
  private:
    int dp(std::vector<std::vector<int>> &arr, size_t i, size_t j,
        ...
        int down = dp(arr, i + 1, j, memo);
        int right = dp(arr, i, j + 1, memo);
        int result = std::min(down, right) - arr[i][j];
        return memo[i][j] = result > 0 ? result : 1;
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        Solution solution;
        auto result = solution.calculateMinimumHP(test);
