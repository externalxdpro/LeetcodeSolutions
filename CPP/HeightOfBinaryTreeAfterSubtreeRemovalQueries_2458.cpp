...
//     after each query. The height of a tree is the number of edges in the
//     longest simple path from the root to some node in the tree.
...
// rooted at node with value 4. The height of the tree is 2 (The path 1 -> 3 ->
// 2).
...
class Solution {
  public:
    std::vector<int> treeQueries(TreeNode *root, std::vector<int> &queries) {
        std::array<int, 100001> maxHeights;
        int                     max = 0;

        std::function<void(TreeNode *, int, bool)> traverse =
            [&](TreeNode *node, int curr, bool lToR) {
                if (node == nullptr) {
                    return;
                }
                if (lToR) {
                    maxHeights[node->val] = max;
                    max                   = std::max(max, curr);
                    traverse(node->left, curr + 1, true);
                    traverse(node->right, curr + 1, true);
                } else {
                    maxHeights[node->val] =
                        std::max(maxHeights[node->val], max);
                    max = std::max(max, curr);
                    traverse(node->right, curr + 1, false);
                    traverse(node->left, curr + 1, false);
                }
            };
