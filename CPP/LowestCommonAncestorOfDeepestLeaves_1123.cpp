...
//     depth of each of its children is d + 1. The lowest common ancestor of a
//     set S of nodes, is the node A with the largest depth such that every node
//     in S is in the subtree with root A.
...
class Solution {
  public:
    TreeNode *lcaDeepestLeaves(TreeNode *root) { return recurse(root).first; }

  private:
    std::pair<TreeNode *, int> recurse(TreeNode *node) {
        if (node == nullptr) {
            return {nullptr, 0};
        }
        auto l = recurse(node->left), r = recurse(node->right);
