...
// n, and moving clockwise from the nth friend brings you to the 1st friend. The
...
//     friends more than once. The last friend you counted leaves the circle and
//     loses the game. If there is still more than one friend in the circle, go
//     back to step 2 starting from the friend immediately clockwise of the
//     friend who just lost and repeat. Else, the last friend in the circle wins
//     the game.
...
int main(int argc, char *argv[]) {
    std::vector<std::pair<int, int>> tests = {{5, 2}, {6, 5}};

    for (auto test : tests) {

        std::cout << findTheWinner(test.first, test.second) << std::endl;
    }
    return 0;
}
