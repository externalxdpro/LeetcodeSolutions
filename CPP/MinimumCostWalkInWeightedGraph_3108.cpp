...
class Solution {
  public:
    std::vector<int> minimumCost(int n, std::vector<std::vector<int>> &edges,
        ...
        for (int i = 0; i < n; i++) {
            if (!visited[i]) {
                cost.push_back(getCost(adj, ids, visited, i, id));
                id++;
            }
        }

        std::vector<int> result;
        for (std::vector<int> &q : query) {
            int src = q[0];
            int dst = q[1];

            if (ids[src] == ids[dst]) {
                result.push_back(cost[ids[src]]);
            } else {
                result.push_back(-1);
            }
        }
  ...
  private:
    int getCost(std::vector<std::vector<std::pair<int, int>>> &adj,
                std::vector<int> &ids, std::vector<bool> &visited, int src,
                int id) {
        std::queue<int> q{{src}};
        visited[src] = true;
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.minimumCost(std::get<0>(test), std::get<1>(test),
                                           std::get<2>(test));
