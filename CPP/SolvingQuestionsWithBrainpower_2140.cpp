...
//         unable to solve questions 1 and 2. If instead, question 0 is skipped
//         and question 1 is solved, you will earn 4 points but you will be
...
class Solution {
  ...
  private:
    long long dp(std::vector<std::vector<int>> &q, int i,
        ...
        long long take = q[i][0] + dp(q, i + q[i][1] + 1, memo);
        long long skip = dp(q, i + 1, memo);
...
int main(int argc, char *argv[]) {
    ...
    for (auto &[test, ans] : tests) {
        auto result = solution.mostPoints(test);
