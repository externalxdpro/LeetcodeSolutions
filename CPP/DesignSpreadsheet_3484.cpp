...
//     to 0. void setCell(String cell, int value) Sets the value of the
//     specified cell. The cell reference is provided in the format "AX" (e.g.,
//     "A1", "B10"), where the letter represents the column (from 'A' to 'Z')
//     and the number represents a 1-indexed row. void resetCell(String cell)
//     Resets the specified cell to 0. int getValue(String formula) Evaluates a
//     formula of the form "=X+Y", where X and Y are either cell references or
...
// with 3 rows and 26 columns spreadsheet.getValue("=5+7"); // returns 12 (5+7)
...
//     equal to 105. Each cell reference consists of a capital letter from 'A'
//     to 'Z' followed by a row number between 1 and rows. At most 104 calls
//     will be made in total to setCell, resetCell, and getValue.

#include <fmt/ranges.h>
#include <vector>

class Spreadsheet {
    std::vector<int> sheet[26];
  ...
  public:
    ...
    void setCell(std::string cell, int value) {
        int col = cell[0] - 'A', row = std::stoi(cell.substr(1)) - 1;
        sheet[col][row] = value;
    }
    ...
    int getValue(std::string formula) {
        ...
        std::string arg1 = formula.substr(1, plus - 1);
        std::string arg2 = formula.substr(plus + 1);
        int val1 = 0, val2 = 0;
        if (arg1[0] >= 'A' && arg1[0] <= 'Z') {
            int col = arg1[0] - 'A', row = std::stoi(arg1.substr(1)) - 1;
            val1 = sheet[col][row];
        } else {
            val1 = std::stoi(arg1);
        }
        if (arg2[0] >= 'A' && arg2[0] <= 'Z') {
            int col = arg2[0] - 'A', row = std::stoi(arg2.substr(1)) - 1;
            val2 = sheet[col][row];
        } else {
            val2 = std::stoi(arg2);
        }
...
int main(int argc, char *argv[]) {
    ...
    fmt::println("{}", obj->getValue("=5+7"));
    obj->setCell("A1", 10);
    fmt::println("{}", obj->getValue("=A1+6"));
    obj->setCell("B2", 15);
    fmt::println("{}", obj->getValue("=A1+B2"));
    obj->resetCell("A1");
    fmt::println("{}", obj->getValue("=A1+B2"));
